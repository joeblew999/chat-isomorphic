...
| `sync-contacts` request | ⚠️ | `manager.request_contacts()` returns OK, but the wait loop for `Received::Contacts` **never resolves** — sat for 6+ min with no events on the stream. Either iOS doesn't send a contact-sync response to a fresh secondary the way presage expects, or the variant translation is missing. Not investigated further; our wrapper doesn't need this. |
